import React from "react";
import playbutton from "../assets/PlayButton.png"

const Result_v1 = ({word}) => {
  return (
    <div className="w-[722px] h-[144px] border flex items-center justify-between">
        <div>

      <h1 className="font-bold font-serif text-7xl w-[293px] h-[77px] border ">{word}</h1>
      <p className="mt-2 font-sans text-3xl text-[#A445ED]">/ˈkiːbɔːd/</p>
        </div>
      <div className="w-[73.47px] h-[75px] border rounded-full bg-[#ead7f8] relative ">
        <img className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  " src={playbutton} alt="play button" />
      </div>
    </div>
  );
};

export default Result_v1;
